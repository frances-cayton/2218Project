...
```{r}
big_X <- Data %>% 
  dplyr::select(one_of('Khodorkovsky', 'PoliticalActivity', 'Loans4Shares', 'Red Director', 'PutinCxn')) %>% 
  as.matrix()
...
big_X <- Data %>% 
  dplyr::select(-one_of('Wealth', 'Level', 'From 2003', 'YrOnList', 'First Year', 'Name', 'Origin', 'Industry', 'Country', 'Russia Rank', 'WorldRank', 'name', 'WIB', 'SecondWave', 'YeltsinCxn', 'Business Captor (state to biz)', 'value',  'My Notes',  'Nezavisimaya', 'Notes (Treisman)', 'PoliticalRole',  'Significant Interest', '2003 Wealth', 'Loans4Shares', 'Second Wave', 'State Captor (biz to state)')) %>% 
  as.matrix()
...
X_treated <- Data %>%
  filter(Privatization == 1) %>%
  dplyr::secect(one_of('Khodorkovsky', 'PoliticalActivity', 'Media Interests', 'Loans4Shares', 'Red Director', 'PutinCxn'))
  dplyr::select(-one_of('Wealth', 'Level', 'From 2003', 'YrOnList', 'First Year', 'Name', 'Origin', 'Industry', 'Country', 'Russia Rank', 'WorldRank', 'name', 'WIB', 'SecondWave', 'YeltsinCxn', 'Business Captor (state to biz)', 'value',  'My Notes', 'Nezavisimaya', 'Notes (Treisman)', 'PoliticalRole',  'Significant Interest', '2003 Wealth', 'Loans4Shares', 'Second Wave', 'State Captor (biz to state)')) %>% 
  as.matrix()
...
matched_data <- rbind(subset(Data, Privatization == 1),
                      subset(Data, Privatization == 0)[match_ids,])
```
...
```{r}
wealth.fit <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` +  `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = matched_data)

summary(wealth.fit)
plot(wealth.fit)

nmwealth.fit <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `Media Interests` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = Data)

summary(nmwealth.fit)
plot(nmwealth.fit)
```

```{r}
wealth.fitlev <- lm(Level ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = matched_data)
...
matched_data2 <- rbind(subset(no.outlier, Privatization == 1),
                      subset(no.outlier, Privatization == 0)[match_ids,])
```


```{r}
wealth.fit2 <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = matched_data2)

summary(wealth.fit2)
plot(wealth.fit2)

nmwealth.fit2 <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `Media Interests` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = no.outlier)

summary(nmwealth.fit2)
plot(nmwealth.fit2)
```

```{r}
wealth.fit2lev <- lm(Level ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = matched_data2)
...
# Persistence Fit:
```{r}
fit3m <- lm(YrOnList ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity + YeltsinCxn, data = matched_data)
...
```
```{r, warning=FALSE, message=FALSE, fig.align='center'}
CI_simed_mod2 <- simulate_CI(wealth.fitlev)
## Plot the results 
...
```{r}
wealth.fitNez<- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` + `Business Captor (state to biz)` + `State Captor (biz to state)` + Nezavisimaya + PutinCxn*PoliticalActivity*YrOnList , data = matched_data)
