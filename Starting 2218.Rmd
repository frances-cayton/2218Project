...
```{r}
wealth.fit2 <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = no.outlier)
...
ggsave("predwealth.jpg", width = 6, height = 4)
```
...
ggplot(CI_simed_mod3, aes(x = YrOnList, y = mean, color = factor(Priv),  fill = factor(Priv))) +
 ...
 xlab("Years on List") + ylab("Average Wealth (in Billions)") +
  ...
  ggtitle("Wealth in Billions by Political Activity") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  geom_line() -> polact


ggsave("polact.jpg", width = 6, height = 4)
```
