...
pred.wealthPolitics
ggsave("predwealthPolitics.jpg", width = 9, height = 5)
```
...
```{r}
subset(CI_simed_mod, PutinCxn == 1) -> subset
```
...
simulate_CI <- function(fitted, nsims = 5000) {
  ...
  CI_simed <- lapply(seq(0, 5, length.out = 25),
                     function (i) {
                       X_Priv <- X
                       X_NoPriv <- X
...
X_Priv[, "Privatization"] <- 1
                       ...
                       X_Priv[, "PutinCxn"] <- 1
                       X_NoPriv[, "PutinCxn"] <- 0
                       X_Priv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 1*15
                       X_NoPriv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 0*15
                       X_Priv[, "PutinCxn:PoliticalActivity"] <- 1
                       X_NoPriv[, "PutinCxn:PoliticalActivity"] <- 0
                       X_Priv[, "PutinCxn:YrOnList"] <- 1*15
                      X_NoPriv[, "PutinCxn:YrOnList"] <- 0
                       X_Priv[, "YrOnList"] <- 15
                       X_NoPriv[, "YrOnList"] <- 15
                       X_Priv[, "Nezavisimaya"] <- i
                       X_NoPriv[, "Nezavisimaya"] <- i
...
## Run the function for our model 
CI_simed_mod9 <- simulate_CI(wealth.fit)
```


```{r, warning=FALSE, message=FALSE, fig.align='center'}
## Plot the results 
ggplot(CI_simed_mod9, aes(x = Nezavisimaya, y = mean, color = factor(Priv), fill = factor(Priv))) +
  geom_ribbon(aes(ymin = p025, ymax = p975, alpha = .05)) +
 xlab("Nezavisimaya") + ylab("Average Wealth (in Billions)") +
    labs(color = "Putin Cxn?", fill = "Putin Cxn?") +
  ggtitle("Wealth in Billions by Putin Connection") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
...
X_Priv[, "Privatization"] <- 1
                       ...
                       X_NoPriv[, "PutinCxn"] <- 0
                       X_Priv[, "PoliticalActivity"] <- 1
                       X_NoPriv[, "PoliticalActivity"] <- 1
                       X_Priv[, "PoliticalActivity:YrOnList"] <- 1*15
                       X_NoPriv[, "PoliticalActivity:YrOnList"] <- 1*15
                       X_Priv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 1*15
                       X_NoPriv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 0*15
                       X_Priv[, "PutinCxn:PoliticalActivity"] <- 1
                       X_NoPriv[, "PutinCxn:PoliticalActivity"] <- 0
...
ggplot(CI_simed_mod9, aes(x = Nezavisimaya, y = mean, color = factor(Priv), fill = factor(Priv))) +
 ...
 xlab("Nezavisimaya") + ylab("Average Wealth (in Billions)") +
  ...
  scale_color_brewer(palette = "Set1") +
  geom_hline(yintercept= 0) +
  geom_line() -> activePower

activePower

ggsave("activePower.jpg", width = 7, height = 5)
```
...
X_Priv[, "Privatization"] <- 1
                       ...
                       X_Priv[, "PutinCxn"] <- 1
                       X_NoPriv[, "PutinCxn"] <- 1
                       X_Priv[, "PoliticalActivity"] <- 0
                       X_NoPriv[, "PoliticalActivity"] <- 0
                         X_Priv[, "PoliticalActivity:YrOnList"] <- 0*15
                       X_NoPriv[, "PoliticalActivity:YrOnList"] <- 0*15
                       X_Priv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 1*15
                       X_NoPriv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 0*15
                       X_Priv[, "PutinCxn:PoliticalActivity"] <- 1
                       X_NoPriv[, "PutinCxn:PoliticalActivity"] <- 0
...
ggplot(CI_simed_mod9, aes(x = Nezavisimaya, y = mean, color = factor(Priv), fill = factor(Priv))) +
  geom_ribbon(aes(ymin = p025, ymax = p975, alpha = .05)) +
 xlab("Nezavisimaya") + ylab("Average Wealth (in Billions)") +
    labs(color = "Putin", fill = "Putin") +
  ggtitle("Wealth in Billions - Not Politically Engaged") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  geom_hline(yintercept= 0) +
  geom_line() -> notactivePOWER

notactivePOWER

ggsave("notactivePOWER.jpg", width = 7, height = 5)
```