  ...
  CI_simed <- lapply(c(0,1),
                     function (i) {
                       ...
                       X_L4SharesNPol <- X
                        ...
                        X_PrivPol[, "Privatization"] <- 1
                       X_NoPrivPol[, "Privatization"] <- 0
                       X_L4SharesPol[, "Privatization"] <- 1
                       X_PrivPol[, "Loans4Shares"] <- 0
                       X_NoPrivPol[, "Loans4Shares"] <- 0
                       X_L4SharesPol[, "Loans4Shares"] <- 1
                       X_PrivPol[, "PoliticalActivity"] <- 1
                       X_NoPrivPol[, "PoliticalActivity"] <- 1
                       X_L4SharesPol[, "PoliticalActivity"] <- 1
                       X_PrivPol[, "PutinCxn"] <- i
                       X_NoPrivPol[, "PutinCxn"] <- i
                       X_PrivPol[, "PutinCxn:PoliticalActivity"] <- i*1
                       X_NoPrivPol[, "PutinCxn:PoliticalActivity"] <- 1*i
                       X_L4SharesPol[, "PoliticalActivity"] <- 1
                       X_L4SharesPol[, "PutinCxn:PoliticalActivity"] <- 1*i
                       
                       
                        X_PrivNPol[, "Privatization"] <- 1
                       X_NoPrivNPol[, "Privatization"] <- 0
                       X_L4SharesNPol[, "Privatization"] <- 1
                       X_PrivNPol[, "Loans4Shares"] <- 0
                       X_NoPrivNPol[, "Loans4Shares"] <- 0
                       X_L4SharesNPol[, "Loans4Shares"] <- 1
                       X_PrivNPol[, "PoliticalActivity"] <- 0
                       X_NoPrivNPol[, "PoliticalActivity"] <- 0
                       X_L4SharesNPol[, "PoliticalActivity"] <- 0
                       X_PrivNPol[, "PutinCxn"] <- i
                       X_NoPrivNPol[, "PutinCxn"] <- i
                       X_PrivNPol[, "PutinCxn:PoliticalActivity"] <- i*0
                       X_NoPrivNPol[, "PutinCxn:PoliticalActivity"] <- 0*i
                       X_L4SharesNPol[, "PoliticalActivity"] <- 0
                       X_L4SharesNPol[, "PutinCxn:PoliticalActivity"] <- 0*i

                     
   
                       # Calculate difference
                       pr_privpol <- colMeans(X_PrivPol %*% t(coef_simed))
                       pr_noprivpol <- colMeans(X_NoPrivPol %*% t(coef_simed))
                        pr_l4s <- colMeans(X_L4SharesPol %*% t(coef_simed))
                       pr_privnp <- colMeans(X_PrivNPol %*% t(coef_simed))
                        pr_noproivnp <- colMeans(X_NoPrivNPol %*% t(coef_simed))
                       pr_l4snp <- colMeans(X_L4SharesNPol %*% t(coef_simed))
   
                       # Reformat the result
                       res_qP <- quantile( pr_privpol, prob = c(0.05,  0.95))
                       res_qNP <- quantile( pr_noprivpol, prob = c(0.05,  0.95))
                        res_l4s<- quantile( pr_l4s, prob = c(0.05,  0.95))
                          res_pnp <- quantile(pr_privnp , prob = c(0.05,  0.95))
                            res_nnp <- quantile(pr_noproivnp, prob = c(0.05,  0.95))
                              res_l4snp <- quantile( pr_l4snp, prob = c(0.05,  0.95))
                        
                       tibble(PutinCxn = i,
                              p025 = res_qP[1],
                              p975 = res_qP[2],
                              mean = mean(pr_privpol)
                              ) -> resP
                       resP$Priv <- c("Privatization")
                       resP$Pol <- c("Politically Active")
              tibble(PutinCxn = i,
                              p025 = res_qNP[1],
                              p975 = res_qNP[2],
                              mean = mean(pr_noprivpol)
                              ) -> resNP
              resNP$Priv <- c("Not Privatization")
              resNP$Pol <- c("Politically Active")
                    tibble(PutinCxn = i,
                              p025 = res_l4s[1],
                              p975 = res_l4s[2],
                              mean = mean( pr_l4s)
                              ) -> resl4s
                       resl4s$Priv <- c("Loans for Shares")
                       resl4s$Pol <- c("Politically Active")
                tibble(PutinCxn = i,
                              p025 = res_pnp[1],
                              p975 = res_pnp[2],
                              mean = mean(pr_privnp)
                              ) -> resPNP
                      resPNP$Priv <- c("Privatization")
                      resPNP$Pol <- c("No")
              tibble(PutinCxn = i,
                              p025 = res_nnp[1],
                              p975 = res_nnp[2],
                              mean = mean(pr_noproivnp)
                              ) -> resNNP
         resNNP$Priv <- c("Not Privatization")
           resNNP$Pol <- c("No")
                    tibble(PutinCxn = i,
                              p025 = res_l4snp[1],
                              p975 = res_l4snp[2],
                     ...
                     resL4SNP$Priv <- c("Loans for Shares")
                 resL4SNP$Pol <- c("No")
              
            
              rbind(resP, resNP, resl4s, resPNP, resNNP, resL4SNP) -> res
                       return(res)
                     }) %>% bind_rows()
  return(CI_simed)
}
...
```{r, warning=FALSE, message=FALSE, fig.align='center'}
CI_simed_mod %>%
ggplot(aes(x = Pol, y = value, color = factor(Priv))) +
  geom_boxplot() +
  facet_wrap(~PutinCxn) +
...
simulate_CI <- function(fitted, nsims = 5000) {
  ...
  CI_simed <- lapply(seq(min(Data$YrOnList), max(Data$YrOnList), length.out = 25),
                     function (i) {
                       ...
                       X_Priv[, "PoliticalActivity"] <- 1
                       X_NoPriv[, "PoliticalActivity"] <- 0
                       X_Priv[, "PutinCxn:PoliticalActivity:YrOnList"] <- i
                       X_NoPriv[, "PutinCxn:PoliticalActivity:YrOnList"] <- 0*i
                       X_Priv[, "PutinCxn:PoliticalActivity"] <- 1
                       X_NoPriv[, "PutinCxn:PoliticalActivity"] <- 0
                       X_Priv[, "PutinCxn:YrOnList"] <- i
                      X_NoPriv[, "PutinCxn:YrOnList"] <- i*0
                      X_NoPriv[, "PoliticalActivity:YrOnList"] <- i*0
                        X_Priv[, "PoliticalActivity:YrOnList"] <- i*1
                       X_Priv[, "YrOnList"] <- i
...
## Run the function for our model 
CI_simed_mod6<- simulate_CI(wealth.fit)
```


```{r}
ggplot(CI_simed_mod6, aes(x = YrOnList, y = mean, color = factor(Priv),  fill = factor(Priv))) +
  geom_ribbon(aes(ymin = p025, ymax = p975, alpha = .05)) + 
 xlab("Years on List") + ylab("Average Wealth (in Billions)") +
    labs(color = "Direct Political Activity?", fill = "Direct Political Activity?") +
  ggtitle("Wealth in Billions by Political Activity - Not Putin Contacts") +
