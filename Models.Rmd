...
```{r}
wealth.fit <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + Nezavisimaya + `Media Interests` + Khodorkovsky + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList + YeltsinCxn, data = Data)
...
```{r}
wealth.fitlev <- lm(Level ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + Nezavisimaya + `Media Interests` + Khodorkovsky + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList + YeltsinCxn, data = Data)
...
```{r}
wealth.fit2 <- lm(Wealth ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `Media Interests` + Khodorkovsky + Nezavisimaya + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = no.outlier)
...
```{r}
wealth.fit2lev <- lm(Level ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + `Media Interests`+ Nezavisimaya + Khodorkovsky + `State Captor (biz to state)` + PutinCxn*PoliticalActivity*YrOnList, data = no.outlier)
...
```{r}
fit3m <- lm(YrOnList ~ Privatization + Loans4Shares + SecondWave + `Always Private` + `Red Director` +  `Business Captor (state to biz)` + Nezavisimaya + `Media Interests` + Khodorkovsky + `State Captor (biz to state)` + PutinCxn*PoliticalActivity + YeltsinCxn, data = Data)
...
```{r}
stargazer(wealth.fit, wealth.fitlev, fit3m)
```
...
## Run the function for our model 
CI_simed_mod8 <- simulate_CI(wealth.fit)
```


```{r, warning=FALSE, message=FALSE, fig.align='center'}
## Plot the results 
ggplot(CI_simed_mod8, aes(x = YrOnList, y = mean, color = factor(Priv), fill = factor(Priv))) +
  geom_ribbon(aes(ymin = p025, ymax = p975, alpha = .2)) +
 xlab("Years on List") + ylab("Average Wealth (in Billions)") +
    labs(color = "Direct Political Activity?", fill = "Direct Political Activity?") +
 ggtitle("Wealth in Billions by Political Activity for Privatization-Era Oligarchs") +
  scale_fill_brewer(palette = "Set1") +
  scale_color_brewer(palette = "Set1") +
  geom_hline(yintercept= 0) +
  geom_line() -> pred.wealthPolitics
pred.wealthPolitics
ggsave("predwealthPolitics.jpg", width = 7, height = 5)
```
...
ggsave("polact.jpg", width = 7, height = 5)
```
...
ggsave("notactive.jpg", width = 7, height = 5)
```
...
For guaging descriptive statistics: 
```{r}
Data %>% 
...
simulate_CI <- function(fitted, nsims = 5000) {
  ...
  CI_simed <- lapply(c(0,1),
                     function (i) {
                       ...
                       X_NoPriv[, "PutinCxn:PoliticalActivity"] <- i
                       ...
                       # Reformat the result
                       res_qP <- quantile(pr_priv, prob = c(0.025,  0.975))
                       res_qNP <- quantile(pr_nopriv, prob = c(0.025,  0.975))
                       tibble(PutinCxn = i,
                              p025 = res_qP[1],
                              p975 = res_qP[2],
                              mean = mean(pr_priv)
...
```{r, warning=FALSE, message=FALSE, fig.align='center'}
ggplot(CI_simed_mod, aes(x = PutinCxn, y = value, color = factor(Priv))) +
  geom_boxplot() +
